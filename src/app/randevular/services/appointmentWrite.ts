...
export async function addServiceToAppointmentAction(payload: AddServicePayload) {
    ...
    try {
        ...
        await db.runTransaction(async (transaction) => {
            const groupAppointmentsSnap = await transaction.get(
                db.collection("appointments").where("groupId", "==", groupId).orderBy("start", "asc")
            );
            
            if (groupAppointmentsSnap.empty) {
                throw new Error("Mevcut randevu grubu bulunamadı.");
            }
            
            const [customerDoc, personnelDoc, serviceOrPackageDoc] = await Promise.all([
                transaction.get(db.collection("customers").doc(customerId)),
                transaction.get(db.collection("personel").doc(personnelId)),
                transaction.get(db.collection(isPackage ? "packages" : "services").doc(serviceId))
            ]);

            if (!customerDoc.exists) throw new Error("Müşteri bulunamadı.");
            if (!personnelDoc.exists) throw new Error("Personel bulunamadı.");
            if (!serviceOrPackageDoc.exists) throw new Error("Hizmet veya paket bulunamadı.");
            ...
            if (duration <= 0) throw new Error("Hizmet süresi geçersiz.");
            
            const appointmentsInGroup = groupAppointmentsSnap.docs
                .map(doc => ({ ...doc.data(), end: (doc.data().end as Timestamp).toDate() } as Appointment));
            
            const lastAppointment = appointmentsInGroup[appointmentsInGroup.length - 1];
            const lastAppointmentEnd = lastAppointment.end;
            const lastAppointmentNotes = lastAppointment.notes || "";
